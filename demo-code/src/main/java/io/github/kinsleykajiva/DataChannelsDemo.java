...
/**
 * Mirrors the Rust {@code data-channels} example (the answer side): the answerer receives
 * an incoming data channel via {@code onDataChannel} and is the side that sends messages
 * (a random 15-character message every 5 seconds). The offerer prints what it receives.
 * This demonstrates handling a data channel that the remote peer created.
 */
public class DataChannelsDemo {
    ...
    public static void main(String[] args) throws Exception {
        ...
        PeerConnection.Observer answererObs = new PeerConnection.Observer() {
            @Override
            public void onDataChannel(int id, String label) {
                System.out.println("[answerer] incoming data channel " + id + " label=" + label);
                            ...
                            Thread.ofVirtual().start(() -> {
                                try {
                                    for (int i = 0; i < 3; i++) {
                                        ...
                                        pair[0].answerer.sendDataChannelText(cid, msg);
                                        System.out.println("[answerer] sent: " + msg);
                                        Thread.sleep(5000);
                                    }
        ...
        pair[0].offerer.setDataChannelCallbacks(dcId,
                ...
                cid -> System.out.println("[offerer] data channel open " + cid),
                cid -> System.out.println("[offerer] data channel closed " + cid));

        pair[0].negotiate();
        System.out.println("Connected. Answerer will send 3 random messages...");
