...
impl MediaEngine {
    ...
    /// Returns the codecs registered (pre-negotiation) for the given kind.
    ///
    /// This is the public accessor used by the FFI layer to enumerate the
    /// supported codec set without depending on the crate-internal fields.
    pub fn registered_codecs(&self, typ: RtpCodecKind) -> Vec<RTCRtpCodecParameters> {
        match typ {
    ...
    /// Returns the registered audio codecs (pre-negotiation).
    pub fn registered_audio_codecs(&self) -> Vec<RTCRtpCodecParameters> {
        self.audio_codecs.clone()
    }

    /// Returns the registered video codecs (pre-negotiation).
    pub fn registered_video_codecs(&self) -> Vec<RTCRtpCodecParameters> {
        self.video_codecs.clone()
    }
